...
public class LevelGenerator : MonoBehaviour
{
    ...
    [SerializeField] int roomCount, randomSeed = 0, scaleFactor;
    [SerializeField] GameObject floorTile, wallLeft, wallRight,
        wallUp, wallDown, lanternPrefab;
    ...
    void DrawRoom(Vector2Int location, Vector2Int size)
    {
        for(int i = 0; i < size.x; i++)
        {
            ...
            if (i > 0 && i % 2 == 0)
            {
                PlaceLantern(location + new Vector2Int(i, 0));
            }
            /*              -------------              */
            /*              -           -              */
            /*              -           -              */
    ...
    {
        GameObject floor = Instantiate(floorTile, new Vector3(location.x * scaleFactor, 0, location.y * scaleFactor), Quaternion.identity);
    }

    void PlaceWallTile(Vector2Int location, GameObject wall)
    {
        GameObject wall1 = Instantiate(wall, new Vector3(location.x * scaleFactor, 0, location.y * scaleFactor), Quaternion.identity);
        GameObject wall2 = Instantiate(wall, new Vector3(location.x * scaleFactor, 3, location.y * scaleFactor), Quaternion.identity);
    }

    void PlaceLantern(Vector2Int location)
    {
        GameObject lantern = Instantiate(lanternPrefab, new Vector3(location.x * scaleFactor, 1, location.y * scaleFactor), Quaternion.identity);
    }
}
